...
def visualize_timeseries(df, resample_periods=["D", "W", "ME", "QE"]):
    """
    ...
    fig.suptitle(
        "Анализ временного ряда XRP (весь период)", fontsize=16, fontweight="bold"
    )
    ...
    fig, axes = plt.subplots(
        len(resample_periods), 1, figsize=(15, 4 * len(resample_periods))
    )
    if len(resample_periods) == 1:
        axes = [axes]

    fig.suptitle(
        "Цена закрытия XRP с разными периодами агрегации",
        fontsize=16,
        fontweight="bold",
    )
    ...
    df_monthly = df_viz["Close"].resample("ME").mean()
    ...
    fig.suptitle(
        "Декомпозиция временного ряда XRP (месячные данные)",
        fontsize=16,
        fontweight="bold",
    )
...
def formulate_hypothesis(df):
    ...
    extreme_events = df_analysis[
        abs(df_analysis["Daily_Return"]) > 0.2
    ]  # Изменения более 20%

    print(f"  - Количество экстремальных скачков цены (>20%): {len(extreme_events)}")
    print(
        f"  - Процент экстремальных событий: {len(extreme_events) / len(df_analysis) * 100:.2f}%"
    )

    if len(extreme_events) > len(df_analysis) * 0.05:  # Более 5% данных
        print(
            f"  ✅ ГИПОТЕЗА: Рекомендуется СЕГМЕНТАЦИЯ данных на периоды до/после значимых событий"
        )
    else:
...
def prepare_data_for_prophet(df, train_size=0.9):
    ...
    print(
        f"📊 Тестовая выборка: {test_samples} записей ({(1 - train_size) * 100:.1f}%)"
    )
    print(
        f"📊 Размер прогноза: {test_samples} дней ({test_samples / len(df_prophet) * 100:.1f}%)"
    )
...
def visualize_forecast(train_df, test_df, forecast, metrics):
    ...
    ax.plot(
        test_df["ds"],
        test_df["y"],
        label="Тестовые данные (истинные)",
        color="green",
        linewidth=2,
    )
        ...
        for s_scale in param_grid["seasonality_prior_scale"]:
            for s_mode in param_grid["seasonality_mode"]:
                ...
                try:
                    ...
                    print(
                        f"  [{iteration}/{total_iterations}] cp={cp_scale}, s={s_scale}, mode={s_mode} → MAPE={mape:.2f}%"
                    )
                    ...
                    print(
                        f"  ❌ Ошибка при обучении с параметрами cp={cp_scale}, s={s_scale}, mode={s_mode}: {e}"
                    )
                    continue
...
def final_training(df_prophet, best_params, forecast_days=30):
    ...
    ax2.plot(
        final_forecast["ds"], final_forecast["trend"], color="green", linewidth=1.5
    )
    ...
    ax3.plot(
        final_forecast["ds"], final_forecast["yearly"], color="orange", linewidth=1.5
    )
    ...
    future_predictions = forecast_future.head(10)[
...
def main():
    ...
    try:
        ...
        visualize_timeseries(df, resample_periods=["W", "ME", "QE"])
        ...
        best_model, best_params, best_metrics, best_forecast = optimize_model(
            train_df, test_df
        )
        ...
        final_model, final_forecast = final_training(
            df_prophet, best_params, forecast_days=30
        )
